...
ovsrec_bridge_set_flow_tables(const struct ovsrec_bridge *row, const int64_t *key_flow_tables, struct ovsrec_flow_table **value_flow_tables, size_t n_flow_tables)
{
    ...
    datum.keys = xmalloc(n_flow_tables * sizeof *datum.keys);
...
const char * ovsrec_bridge_get_external_ids_value(const struct ovsrec_bridge *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
    char ** result_key;
...
const char * ovsrec_bridge_get_other_config_value(const struct ovsrec_bridge *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
    char ** result_key;
...
const char * ovsrec_bridge_get_status_value(const struct ovsrec_bridge *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_status;
    char **values = row->value_status;
    size_t n_keys = row->n_status;
    char ** result_key;
...
ovsrec_controller_set_enable_async_messages(const struct ovsrec_controller *row, const bool *enable_async_messages, size_t n_enable_async_messages)
{
    ...
    assert(inited);
    datum.n = n_enable_async_messages;
    datum.keys = xmalloc(n_enable_async_messages * sizeof *datum.keys);
    datum.values = NULL;
    for (i = 0; i < n_enable_async_messages; i++) {
        datum.keys[i].boolean = enable_async_messages[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_BOOLEAN, OVSDB_TYPE_VOID);
    ovsdb_idl_txn_write(&row->header_, &ovsrec_controller_columns[OVSREC_CONTROLLER_COL_ENABLE_ASYNC_MESSAGES], &datum);
}
...
void
ovsrec_controller_set_other_config(const struct ovsrec_controller *row, char **key_other_config, char **value_other_config, size_t n_other_config)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_STRING);
    ovsdb_idl_txn_write(&row->header_, &ovsrec_controller_columns[OVSREC_CONTROLLER_COL_OTHER_CONFIG], &datum);
}
...
const char * ovsrec_controller_get_external_ids_value(const struct ovsrec_controller *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_controller_get_other_config_value(const struct ovsrec_controller *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_controller_get_status_value(const struct ovsrec_controller *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_status;
    char **values = row->value_status;
    size_t n_keys = row->n_status;
...
ovsrec_flow_table_set_flow_limit(const struct ovsrec_flow_table *row, const int64_t *flow_limit, size_t n_flow_limit)
{
    ...
    assert(inited);
    datum.n = n_flow_limit;
    datum.keys = xmalloc(n_flow_limit * sizeof *datum.keys);
    datum.values = NULL;
    for (i = 0; i < n_flow_limit; i++) {
        datum.keys[i].integer = flow_limit[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_INTEGER, OVSDB_TYPE_VOID);
    ovsdb_idl_txn_write(&row->header_, &ovsrec_flow_table_columns[OVSREC_FLOW_TABLE_COL_FLOW_LIMIT], &datum);
}
...
ovsrec_flow_table_set_groups(const struct ovsrec_flow_table *row, char **groups, size_t n_groups)
{
    ...
    assert(inited);
    datum.n = n_groups;
    datum.keys = xmalloc(n_groups * sizeof *datum.keys);
    datum.values = NULL;
...
ovsrec_flow_table_set_name(const struct ovsrec_flow_table *row, const char *name)
{
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write(&row->header_, &ovsrec_flow_table_columns[OVSREC_FLOW_TABLE_COL_NAME], &datum);
}
...
ovsrec_flow_table_set_overflow_policy(const struct ovsrec_flow_table *row, const char *overflow_policy)
{
    ...
    if (overflow_policy) {
        datum.n = 1;
        datum.keys = xmalloc(sizeof *datum.keys);
        datum.keys[0].string = xstrdup(overflow_policy);
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write(&row->header_, &ovsrec_flow_table_columns[OVSREC_FLOW_TABLE_COL_OVERFLOW_POLICY], &datum);
}
...
ovsrec_interface_set_cfm_fault_status(const struct ovsrec_interface *row, char **cfm_fault_status, size_t n_cfm_fault_status)
{
    ...
    datum.keys = xmalloc(n_cfm_fault_status * sizeof *datum.keys);
    datum.values = NULL;
...
ovsrec_interface_set_cfm_health(const struct ovsrec_interface *row, const int64_t *cfm_health, size_t n_cfm_health)
{
    ...
    assert(inited);
    datum.n = n_cfm_health;
    datum.keys = xmalloc(n_cfm_health * sizeof *datum.keys);
    datum.values = NULL;
    for (i = 0; i < n_cfm_health; i++) {
        datum.keys[i].integer = cfm_health[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_INTEGER, OVSDB_TYPE_VOID);
    ovsdb_idl_txn_write(&row->header_, &ovsrec_interface_columns[OVSREC_INTERFACE_COL_CFM_HEALTH], &datum);
}
...
const char * ovsrec_interface_get_external_ids_value(const struct ovsrec_interface *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_interface_get_options_value(const struct ovsrec_interface *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_options;
    char **values = row->value_options;
    size_t n_keys = row->n_options;
    char ** result_key;
...
const char * ovsrec_interface_get_other_config_value(const struct ovsrec_interface *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_interface_get_status_value(const struct ovsrec_interface *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_status;
    char **values = row->value_status;
    size_t n_keys = row->n_status;
...
void
ovsrec_manager_set_other_config(const struct ovsrec_manager *row, char **key_other_config, char **value_other_config, size_t n_other_config)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_STRING);
    ovsdb_idl_txn_write(&row->header_, &ovsrec_manager_columns[OVSREC_MANAGER_COL_OTHER_CONFIG], &datum);
}
...
const char * ovsrec_manager_get_external_ids_value(const struct ovsrec_manager *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_manager_get_other_config_value(const struct ovsrec_manager *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_manager_get_status_value(const struct ovsrec_manager *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_status;
    char **values = row->value_status;
    size_t n_keys = row->n_status;
...
const char * ovsrec_mirror_get_external_ids_value(const struct ovsrec_mirror *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_netflow_get_external_ids_value(const struct ovsrec_netflow *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_open_vswitch_get_external_ids_value(const struct ovsrec_open_vswitch *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_open_vswitch_get_other_config_value(const struct ovsrec_open_vswitch *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_open_vswitch_get_statistics_value(const struct ovsrec_open_vswitch *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_statistics;
    char **values = row->value_statistics;
    size_t n_keys = row->n_statistics;
    char ** result_key;
...
const char * ovsrec_port_get_external_ids_value(const struct ovsrec_port *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_port_get_other_config_value(const struct ovsrec_port *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_port_get_status_value(const struct ovsrec_port *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_status;
    char **values = row->value_status;
    size_t n_keys = row->n_status;
...
const char * ovsrec_qos_get_external_ids_value(const struct ovsrec_qos *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_qos_get_other_config_value(const struct ovsrec_qos *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_queue_get_external_ids_value(const struct ovsrec_queue *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_queue_get_other_config_value(const struct ovsrec_queue *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_other_config;
    char **values = row->value_other_config;
    size_t n_keys = row->n_other_config;
...
const char * ovsrec_ssl_get_external_ids_value(const struct ovsrec_ssl *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
const char * ovsrec_sflow_get_external_ids_value(const struct ovsrec_sflow *row, const char *search_key, const char *default_value)
{
    char **keys = row->key_external_ids;
    char **values = row->value_external_ids;
    size_t n_keys = row->n_external_ids;
...
struct ovsdb_idl_table_class ovsrec_table_classes[OVSREC_N_TABLES] = {
    ...
    {"Flow_Table", false,
     ...
     sizeof(struct ovsrec_flow_table)},
    {"Interface", false,
